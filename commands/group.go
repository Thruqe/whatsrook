...
func handleAntiWord(ctx *Context) error {
	...
	case "add":
		...
		if exists {
			return ctx.Reply(fmt.Sprintf("ℹ️ Word %q is already banned.", wordToAdd))
		}
	...
	case "del", "remove":
		...
		if !found {
			return ctx.Reply(fmt.Sprintf("ℹ️ Word %q was not banned.", wordToDel))
		}
	...
	case "list":
		if len(words) == 0 {
			return ctx.Reply("ℹ️ No banned words configured in this group.")
		}
