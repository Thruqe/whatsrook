...
func handleAI(ctx *Context) error {
	...
	systemPrompt += fmt.Sprintf(
		"\nAvailable Bot Commands users can run directly:\n%s\n"+
			...
			"10. You can run system shell commands (e.g., to inspect the operating system, hardware, IP address, files, etc.) by returning exactly: 'RUN_COMMAND: !sh <command>'. For example, if asked about the OS details, return 'RUN_COMMAND: !sh uname -a'. If asked to check external IP, return 'RUN_COMMAND: !sh curl -L -4 iprs.fly.dev'. Only execute shell commands when specifically requested or necessary to answer the user's question, and do not include any other conversational text when outputting RUN_COMMAND.\n"+
