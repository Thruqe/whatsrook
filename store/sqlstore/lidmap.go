...
const (
	deleteExistingLIDMappingQuery = `DELETE FROM lid_map WHERE (lid<>$1 AND pn=$2)`
	putLIDMappingQuery            = `
		INSERT INTO lid_map (lid, pn)
		VALUES ($1, $2)
		ON CONFLICT (lid) DO UPDATE SET pn=excluded.pn WHERE lid_map.pn<>excluded.pn
	`
	getLIDForPNQuery       = `SELECT lid FROM lid_map WHERE pn=$1`
	getPNForLIDQuery       = `SELECT pn FROM lid_map WHERE lid=$1`
	getAllLIDMappingsQuery = `SELECT lid, pn FROM lid_map`
)
...
func (s *CachedLIDMap) GetManyLIDsForPNs(ctx context.Context, pns []types.JID) (map[types.JID]types.JID, error) {
	...
	if s.db.Dialect == dbutil.Postgres && PostgresArrayWrapper != nil {
		res = convertLIDRow.NewRowIter(s.db.Query(
			ctx,
			`SELECT lid, pn FROM lid_map WHERE pn = ANY($1)`,
		...
		res = convertLIDRow.NewRowIter(s.db.Query(
			ctx,
			fmt.Sprintf(`SELECT lid, pn FROM lid_map WHERE pn IN (%s)`, strings.Join(placeholders, ",")),
