...
import (
	...
	"whatsrook/wa-core/proto/waHistorySync"
	"whatsrook/wa-core/proto/waVnameCert"
...
func (cli *Client) GetUserDevices(ctx context.Context, jids []types.JID) ([]types.JID, error) {
	...
	if len(jidsToSync) > 0 {
		...
		for _, user := range list.GetChildren() {
			...
			putBoundedCache(ensureMap(&cli.userDevicesCache), jid, deviceCache{devices: userDevices, dhash: participantListHashV2(userDevices)}, maxUserDeviceCacheEntries)
